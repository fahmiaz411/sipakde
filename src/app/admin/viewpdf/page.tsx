...
export default function Page() {
  ...
  const { toastDevelopment } = useGlobalContext();
  ...
  const checkStatus = async () => {
    try {
      if (pdfDetails.status === "pending") {
        const query = new URLSearchParams();

        query.append("document_id", String(pdfDetails.id));
        query.append("status", "onview");

        const res = await fetch(
          `/api/documents/change-status?${query.toString()}`,
          {
            method: "PATCH",
          }
        );
  ...
  const handleAction = async (action: string) => {
    try {
      if (["pending", "onview"].includes(pdfDetails.status)) {
        const query = new URLSearchParams();

        query.append("document_id", String(pdfDetails.id));
        query.append("status", action);
      ...
      <div className={styles.container}>
        ...
        <div className={styles.detailsContainer}>
          ...
          <div className={styles.actionsContainer}>
            ...
            {["pending", "onview"].includes(pdfDetails.status) && (
              <>
                <button
